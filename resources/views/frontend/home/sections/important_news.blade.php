    ...
    <div class="site-container">
        ...
        <div class="archive-grid">
            @forelse ($importantPosts ?? collect() as $post)
                <article class="archive-card"><a href="{{ route('posts.show', $post->slug) }}"><img class="archive-card-img" src="{{ $imageUrl($post->featured_image) }}" alt="{{ $post->title }}" loading="lazy"><div class="archive-card-body"><span class="card-cat">{{ $post->category?->title ?: 'خبر' }}</span><h2>{{ $post->title }}</h2><p>{{ $post->excerpt ?: Str::limit(strip_tags($post->body), 120) }}</p></div></a></article>
            @empty
