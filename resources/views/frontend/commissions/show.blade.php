...
@section('meta_description', Str::limit(strip_tags($commission->description), 160))
...
<section class="site-container">
    <div class="news-single-layout">
        <article class="news-single-main">
            @if($commission->image)
                <div class="news-single-cover"><img src="{{ Storage::url($commission->image) }}" alt="{{ $commission->title }}" loading="lazy"></div>
            @endif
            <div class="news-single-body">
                <h2>{{ $commission->title }}</h2>
                <div>{!! $commission->description ?: '<p>توضیحات این کمیسیون هنوز تکمیل نشده است.</p>' !!}</div>
            </div>
            <div class="admin-panel-card mt-4">
                <h3>وظایف کمیسیون</h3>
                @forelse($commission->activeTasks as $task)
                    <div class="related-post-item" style="align-items:flex-start">
                        <div>
            ...
            <div class="admin-panel-card mt-4">
                ...
                @forelse($commission->members ?? [] as $member)
                    <span class="post-tag">{{ $member['name'] ?? '' }}</span>
                @empty
            ...
            @if(!empty($commission->attachments))
                <div class="admin-panel-card mt-4">
                    ...
                    @foreach($commission->attachments as $file)
                        <p><a class="post-tag" href="{{ Storage::url($file['path'] ?? '') }}" target="_blank">{{ $file['name'] ?? 'دانلود فایل' }}</a></p>
                    @endforeach
                </div>
            @endif
            <div class="admin-panel-card mt-4">
                ...
                @forelse($commission->publishedSessions as $session)
                    <div class="related-post-item" style="align-items:flex-start">
                        <div>
                            ...
                            <p>{{ Str::limit(strip_tags($session->description), 140) }}</p>
                            @if($session->minutes_file)
                                <a class="post-tag" href="{{ Storage::url($session->minutes_file) }}" target="_blank">دانلود صورتجلسه</a>
                            @endif
                        </div>
                    </div>
                @empty
        ...
        <aside class="news-sidebar">
            <div class="news-sidebar-card">
                <h4>اطلاعات کمیسیون</h4>
                <p>تعداد اعضا: {{ count($commission->members ?? []) }}</p>
                <p>تعداد جلسات: {{ $commission->publishedSessions->count() }}</p>
                <a class="tab-pill" href="{{ route('commissions.index') }}">بازگشت به کمیسیون‌ها</a>
            </div>
        </aside>
    </div>
</section>
