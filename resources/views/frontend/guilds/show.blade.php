    ...
    <div class="site-container guild-layout">
        ...
        <div>
            ...
            @if($union->isSectionEnabled('show_prices', false))
                <section class="guild-section guild-section-alt" id="guild-prices"><h3 class="guild-section-title">نرخ‌نامه اختصاصی اتحادیه</h3><div class="price-table"><table><thead><tr><th>عنوان</th><th>نوع</th><th>قیمت</th><th>تاریخ بروزرسانی</th></tr></thead><tbody>@forelse($union->prices as $price)<tr><td>{{ $price->title }}</td><td>{{ $price->type ?: 'عمومی' }}</td><td>{{ $price->price ? number_format((float) $price->price).' '.$price->currency : 'اعلام نشده' }}</td><td>{{ $price->updated_on ? jalali_date($price->updated_on) : '—' }}</td></tr>@empty<tr><td colspan="4">نرخی برای نمایش ثبت نشده است.</td></tr>@endforelse</tbody></table></div></section>
            @endif
