...
<form class="admin-panel-card admin-form" action="{{ route('admin.advertisements.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label" for="starts_at">زمان شروع</label><input class="form-control" id="starts_at" name="starts_at" type="datetime-local" value="{{ old('starts_at', now()->format('Y-m-d\TH:i')) }}" required></div>
        <div class="col-md-4"><label class="form-label" for="expires_at">زمان پایان</label><input class="form-control" id="expires_at" name="expires_at" type="datetime-local" value="{{ old('expires_at') }}"></div>
