...
<form class="admin-panel-card admin-form" action="{{ route('admin.advertisements.update', $advertisement) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label" for="starts_at">زمان شروع</label><input class="form-control js-jalali-datetime" id="starts_at" name="starts_at" type="text" dir="ltr" value="{{ old('starts_at', jalali_form_datetime($advertisement->starts_at)) }}" required></div>
        <div class="col-md-4"><label class="form-label" for="expires_at">زمان پایان</label><input class="form-control js-jalali-datetime" id="expires_at" name="expires_at" type="text" dir="ltr" value="{{ old('expires_at', jalali_form_datetime($advertisement->expires_at)) }}"></div>
