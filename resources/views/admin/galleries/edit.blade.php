...
<form class="admin-panel-card admin-form" action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label" for="category_id">شناسه دسته‌بندی</label><input class="form-control" id="category_id" name="category_id" type="number" min="1" value="{{ old('category_id', $gallery->category_id) }}"></div>
        ...
        <div class="col-md-4"><label class="form-label" for="published_at">تاریخ انتشار</label><input class="form-control js-jalali-datetime" id="published_at" name="published_at" type="text" dir="ltr" value="{{ old('published_at', jalali_form_datetime($gallery->published_at)) }}"></div>
        <div class="col-md-4"><label class="form-label" for="sort_order">ترتیب نمایش</label><input class="form-control" id="sort_order" name="sort_order" type="number" min="0" value="{{ old('sort_order', $gallery->sort_order) }}"></div>
        ...
        <div class="col-12"><label class="form-label" for="description">توضیحات</label><textarea class="form-control" id="description" name="description" rows="4">{{ old('description', $gallery->description) }}</textarea></div>
                ...
                @foreach ($gallery->images as $image)
                    <div class="col-md-4">
                        <div class="border rounded p-2 h-100">
                            ...
                            <label class="form-label mt-2" for="sort_{{ $image->id }}">ترتیب</label>
                            <input class="form-control" id="sort_{{ $image->id }}" name="existing_images[{{ $image->id }}][sort_order]" type="number" min="0" value="{{ old('existing_images.'.$image->id.'.sort_order', $image->sort_order) }}">
