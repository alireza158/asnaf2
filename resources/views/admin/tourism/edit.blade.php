...
<form class="admin-panel-card admin-form" action="{{ route('admin.tourism.update', $place) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label" for="type">نوع گردشگری</label><select class="form-control" id="type" name="type" required><option value="nature" @selected(old('type', $place->type) === 'nature')>طبیعت‌گردی</option><option value="historic" @selected(old('type', $place->type) === 'historic')>تاریخی</option><option value="shop" @selected(old('type', $place->type) === 'shop')>بازار و خرید</option></select></div>
        ...
        <div class="col-md-6"><label class="form-label" for="image">مسیر تصویر کارت</label><input class="form-control" id="image" name="image" value="{{ old('image', $place->image) }}" dir="ltr" placeholder="assets/img/asnaf-gorgan-default.jpg"></div>
