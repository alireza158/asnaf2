...
<form class="admin-panel-card admin-form" action="{{ route('admin.tourism.update', $place) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6"><label class="form-label" for="image">تصویر کارت</label><input class="form-control" id="image" name="image" type="file" accept="image/*">@if ($place->image)<small><a href="{{ Storage::url($place->image) }}" target="_blank">مشاهده تصویر کارت فعلی</a></small>@endif</div>
